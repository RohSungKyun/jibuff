...
**Max interview rounds:** 8 (rtc), 5 (quick)
...
Each round asks one focused question to keep the interview readable. The question includes `a)`, `b)`, and `c)` options plus a direct-input path for answers that do not fit the presets.

MCP clients can request the same round as a structured payload with
`response_format: "json"`. In that mode, `jibuff_interview` returns a
transport-neutral `jibuff.interview.question` object containing the question,
three selectable options, `allow_other: true`, and `fallback_text`. This mirrors
the interaction shape of tools such as OMX `omx question` without requiring an
OMX runtime; clients that cannot render a structured UI can display
`fallback_text` and continue with the same `session_id` and `revision`.
